...
public class TCPClient : MonoBehaviour
{
    ...
    // removed ts_left, ts_right
    public async void SendPVImageAsync(byte[] LRFImage)
    {
        if (!lastMessageSent) return;
        lastMessageSent = false;
        try
    ...
    public async void SendPVTxtAsync(byte[] LRFImage)
    {
        if (!lastMessageSent) return;
        lastMessageSent = false;
        try
    ...
    public async void MakeVid()
    {
        try
        {
            // Write header
            dw.WriteString("m"); // header "f"
            // Send out
            await dw.StoreAsync();
            await dw.FlushAsync();
        }
